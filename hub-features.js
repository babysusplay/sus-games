...
(() => {
  ...
  /* Quiz-style direct chat. Existing Supabase account, message table and storage are reused. */
  const chatCss=document.createElement('style');chatCss.id='sgQuizStyleChat';chatCss.textContent=`
    ...
    .sg-chat-row{display:flex;align-items:flex-end;gap:8px;width:100%;padding:2px 4px}.sg-chat-row.me{justify-content:flex-end}.sg-chat-row.them{justify-content:flex-start}
    .sg-chat-avatar{width:42px;height:42px;flex:0 0 42px;border-radius:50%;object-fit:cover;background:#242833;border:1px solid rgba(255,255,255,.10)}.sg-chat-avatar.spacer{visibility:hidden}
    .sg-chat-content{max-width:min(72%,620px);display:flex;flex-direction:column;gap:3px}.sg-chat-row.me .sg-chat-content{align-items:flex-end}
    .sg-chat-author{font-size:13px;font-weight:800;color:#fff;line-height:1.2;padding:0 4px}.sg-chat-bubble{padding:9px 12px;border-radius:12px;background:#242833;color:#fff;line-height:1.35;word-break:break-word}.sg-chat-row.me .sg-chat-bubble{background:#30343e}
    .sg-chat-time{display:none;color:#777d8a;font-size:10px;padding:0 4px}.sg-chat-time.show{display:block}
    .sg-chat-image{display:block;max-width:240px;max-height:300px;border-radius:10px;object-fit:cover;cursor:pointer}
    .sg-chat-image-picker-row{display:flex;align-items:center;margin-top:10px;margin-bottom:6px}.sg-chat-plus{width:58px;height:58px;flex:0 0 58px;border:1px solid rgba(255,255,255,.11);border-radius:12px;background:rgba(255,255,255,.055);color:#fff;font-size:28px;font-weight:700;display:grid;place-items:center;cursor:pointer}.sg-chat-plus:hover{background:rgba(255,255,255,.12)}
    .sg-chat-compose-row{display:flex;align-items:flex-end;gap:8px}.sg-chat-compose-row textarea{flex:1;min-height:58px;max-height:140px;resize:vertical;background:#0f1117!important;color:#fff!important;border:1px solid rgba(255,255,255,.09)!important;border-radius:12px!important;padding:13px!important}.sg-chat-compose-row input[type=file]{display:none!important}
    .sg-chat-send{min-height:58px!important;border-radius:12px!important;padding:0 20px!important}.sg-chat-note{color:#858b98;font-size:12px;margin-top:8px}
    @media(max-width:700px){.sg-chat-content{max-width:80%}.sg-chat-avatar{width:36px;height:36px;flex-basis:36px}.sg-chat-image{max-width:72vw}.sg-chat-plus{width:50px;height:50px;flex-basis:50px}.sg-chat-compose-row textarea{min-height:50px}.sg-chat-send{min-height:50px!important;padding:0 14px!important}}
  ...
  function renderMessages(rows,meId,meProfile,targetProfile){
    ...
    out.innerHTML=rows.map(m=>{
      const mine=String(m.sender_id)===String(meId),sender=mine?meProfile:targetProfile,name=sender?.display_name||(mine?'You':'Player'),avatar=sender?.avatar_url||fallbackAvatar(name),dt=new Date(m.created_at);
      ...
      const showIdentity=String(m.sender_id)!==String(lastSender);lastSender=m.sender_id;
  ...
  async function openExactChat(targetId){
    ...
    modal.querySelector('.sg-body').innerHTML=`<div class="sg-chat"><div id="sgMessages" class="sg-messages"></div><div class="sg-chat-image-picker-row"><input id="sgChatImageInput" type="file" accept="image/*"><button id="sgChatPlus" class="sg-chat-plus" type="button" title="Send picture">+</button></div><div class="sg-chat-compose-row"><textarea id="sgMessageInput" maxlength="1000" placeholder="Write a message..."></textarea><button id="sgChatSend" class="sg-btn sg-chat-send" type="button">Send</button></div><div class="sg-chat-note">Direct chat is available only between mutual followers.</div></div>`;
