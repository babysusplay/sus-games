...
(() => {
  ...
  const chatCss=document.createElement('style');chatCss.id='sgQuizStyleChat';chatCss.textContent=`
    ...
    .sg-chat-image-picker-row{display:flex;align-items:center;margin-top:0;margin-bottom:6px}.sg-chat-plus{width:48px;height:48px;flex:0 0 48px;border:1px solid rgba(255,255,255,.11);border-radius:12px;background:rgba(255,255,255,.055);color:#fff;font-size:24px;font-weight:700;display:grid;place-items:center;cursor:pointer}.sg-chat-plus:hover{background:rgba(255,255,255,.12)}
    .sg-chat-compose-row{display:flex;align-items:flex-end;gap:8px}.sg-chat-compose-row textarea{flex:1;min-height:58px;max-height:100px;resize:vertical;background:#0f1117!important;color:#fff!important;border:1px solid rgba(255,255,255,.09)!important;border-radius:12px!important;padding:13px!important;line-height:1.45}.sg-chat-compose-row input[type=file]{display:none!important}
    .sg-chat-send{min-height:48px!important;border-radius:12px!important;padding:0 16px!important;background:#fff!important;color:#111!important}.sg-chat-send:hover{background:#e9e9e9!important}.sg-chat-note{color:#858b98;font-size:12px;margin-top:8px}
  ...
  async function getAdminFlag(id){const sb=getSharedSupabase();if(!sb||!id)return false;try{const me=await getMe();if(me&&String(me.id)===String(id)){const {data,error}=await sb.rpc('is_admin');if(!error)return data===true}const {data,error}=await sb.from('admin_users').select('user_id').eq('user_id',id).maybeSingle();return !error&&!!data}catch{return false}}
  ...
  async function loadChatExact(targetId){
    const sb=getSharedSupabase(),me=await getMe();if(!sb||!me)return;
    const [meProfile,targetProfile,meAdmin,targetAdmin]=await Promise.all([getProfile(me.id),getProfile(targetId),getAdminFlag(me.id),getAdminFlag(targetId)]);
    ...
    renderMessages(data||[],me.id,meProfile,targetProfile,meAdmin,targetAdmin);
  }
  ...
  async function sendImageExact(file,targetId){
    ...
    const ext=(file.name.split('.').pop()||'jpg').toLowerCase(),path=`${me.id}/${crypto.randomUUID()}.${ext}`;
